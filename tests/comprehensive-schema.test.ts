...
describe('Comprehensive Schema Generation', () => {
  ...
  it('should generate comprehensive models with enums and relations', async () => {
    ...
    expect(
      existsSync(path.join(outputPath, 'models', 'Profile.model.ts')),
    ).toBe(true);
    expect(existsSync(path.join(outputPath, 'models', 'Tag.model.ts'))).toBe(
      true,
    );
    expect(existsSync(path.join(outputPath, 'enums', 'Role.enum.ts'))).toBe(
      true,
    );
    expect(existsSync(path.join(outputPath, 'enums', 'Status.enum.ts'))).toBe(
      true,
    );
  ...
  it('should generate enum imports and validators', async () => {
    ...
    expect(userContent).toContain(
      'import { Role, Status } from "../enums/index.js"',
    );
